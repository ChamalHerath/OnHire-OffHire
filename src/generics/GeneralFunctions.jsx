...
function formatStandardDate(d) {//Date() to DD/MM/YYYY HH:mm:ss
    return moment(d).format("DD/MM/YYYY HH:mm:ss")
}


function formatDateStrFromTime(t, type) {//Create a date from a given "HH:mm" or "mm:ss" time
    d = formatDateFromTime(t, type)
    d = moment(d).format("DD/MM/YYYY HH:mm:ss")
    return d
}

function formatDateFromTime(t, type) {//Create a date from a given "HH:mm" or "mm:ss" time
    if (isEmpty(t)) { t = "00:00" }
...
function formatImageForAPI(image) {
    if (image && image.blob) {
        return image.blob.split(",")[1]
    }
